...
const Map = ({ apiKey }: Props) => {
  ...
  const handleInputChange = (event: { preventDefault: () => void }) => {
    ...
    const selectedCnic = cnicCoordinates[inputValue];

    if (selectedCnic) {
      // Set the selected CNIC's coordinates
      //   setSelectedCnicCoordinates(selectedCnic);
    }
  };
  ...
  const cnicCoordinates: CnicCoordinates = {
    "35200111122221": {
      originLat: 31.45,
      originLng: 74.267817,
      destinationLat: 31.4815,
      destinationLng: 74.303,
    },
    "35200111122222": {
      originLat: 31.45,
      originLng: 74.267817,
      destinationLat: 31.428263,
      destinationLng: 74.267817,
      // 31.428263. WAPDA Town Longitude: 74.267817.
    },
  };
  const [inputValue, setInputValue] = useState<string>("");
  const [selectedCnicCoordinates, setSelectedCnicCoordinates] = useState<
    [number, number, number, number]
  >([31.45, 74.267817, 31.428263, 74.267817]);

  const [points, setPoints] = useState<string>("35200111122221");

  useEffect(() => {}, []);

  // Add more CNICs and coordinates here
                ...
                <DirectionsService
                  options={{
                    destination: new google.maps.LatLng(
                      cnicCoordinates["35200111122221"].destinationLat,
                      cnicCoordinates["35200111122221"].destinationLng
                    ),
                    origin: new google.maps.LatLng(
                      cnicCoordinates["35200111122221"].originLat,
                      cnicCoordinates["35200111122221"].originLng
                    ),
                    travelMode: google.maps.TravelMode.DRIVING,
                    provideRouteAlternatives: true, // Enable route alternatives
                  }}
                  callback={(response) => {
                    if (response !== null) {
